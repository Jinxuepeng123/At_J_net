...
if __name__ == '__main__':
    ...
    print(depths.shape)
    print(images.shape)
    depths = np.array(depths)  #感觉这两句可有可无，不影响代码的实现
    images = np.array(images)

    # 裁剪其使其能放入AtJ网络
    depths = depths[:, trim_size:160 - trim_size, trim_size:160 - trim_size]
    images = images[:, :, trim_size:160 - trim_size, trim_size:160 - trim_size]
    print(depths.shape)
    print(images.shape)
    ...
    for i in range(length):
        ...
        depth = depth / m   #归一化
        image = images[i]
        print('dealing:' + str(i) + '.png')  #str()把数字转化为字符
        ...
        r = Image.fromarray(images[i][0]).convert('L')  #将数组变为图像，L代表灰度图
