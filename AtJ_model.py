...
class BottleneckBlock(nn.Module):
    def __init__(self, in_planes, out_planes, dropRate=0.0):
        ...
        inter_planes = out_planes * 4     #inter_planes=1792
        self.bn1 = nn.BatchNorm2d(in_planes)
...
class Dense_decoder(nn.Module):
    ...
    def forward(self, x, x1, x2, x4, activation=None):
        ...
        x1010 = self.upsample(self.relu(self.conv1010(x101)), size=shape_out, mode='bilinear', align_corners=True)  #torch.nn.functional.upsample() 双线性插值，输入和输出张量由其角像素的中心点对齐，从而保留角像素处的值。
